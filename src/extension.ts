...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    "extension.fetchBackstageEntities",
...
async function fetchAndProcessEntities() {
  const backstageBaseUrl = vscode.workspace
    .getConfiguration()
    .get("backstageCatalogHelper.baseUrl") as string;
  debug =
    vscode.workspace.getConfiguration().get("backstageCatalogHelper.debug") ===
    true;

  if (!backstageBaseUrl) {
    vscode.window.showErrorMessage(
      "Backstage base URL is not configured. Please set it in the settings."
    );
    return;
  }
  ...
  try {
    ...
    vscode.window.showInformationMessage(
      "Backstage entities processed successfully!"
    );
  } catch (error: unknown) {
    vscode.window.showErrorMessage(
      "Failed to fetch Backstage entities: " + (error as Error).message
    );
  }
}
...
async function fetchBackstageEntities(apiUrl: string): Promise<void> {
  ...
  for (const entity of response.data) {
    ...
    switch (entity.kind) {
      case "System":
        backstageSuggestions.systems.set(
          entity.metadata.name,
          entity.metadata.description || ""
        );
        break;
      case "Group":
        backstageSuggestions.groups.set(
          entity.metadata.name,
          entity.metadata.description || ""
        );
        break;
      case "Component":
        backstageSuggestions.components.set(
          entity.metadata.name,
          entity.metadata.description || ""
        );
        break;
    ...
    if (entity.spec) {
      ...
      if (entity.spec.lifecycle)
        backstageSuggestions.lifecycles.add(entity.spec.lifecycle);
    }
  }
}
...
function showDebugInfo(data: any) {
  vscode.workspace
    .openTextDocument({ content: JSON.stringify(data, null, 2) })
    .then((doc) => {
          ...
          switch (key) {
            case "system":
              return provideFilteredMapSuggestions(
                backstageSuggestions.systems,
                "System",
                partialValue
              );
            case "kind":
              return provideFilteredSetSuggestions(
                backstageSuggestions.kinds,
                "Kind",
                partialValue
              );
            case "owner":
              return provideFilteredMapSuggestions(
                backstageSuggestions.groups,
                "Group",
                partialValue
              );
            case "type":
              return provideFilteredSetSuggestions(
                backstageSuggestions.types,
                "Type",
                partialValue
              );
            case "lifecycle":
              return provideFilteredSetSuggestions(
                backstageSuggestions.lifecycles,
                "Lifecycle",
                partialValue
              );
            case "component":
              return provideFilteredMapSuggestions(
                backstageSuggestions.components,
                "Component",
                partialValue
              );
            case "resource":
...
function provideFilteredMapSuggestions(
...
function provideFilteredSetSuggestions(
  set: Set<string>,
  kind: string,
  partialValue: string
): vscode.CompletionItem[] {
